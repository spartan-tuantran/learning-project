...
import com.alext.rtree.core.builder.ImmutableMultiIndexedRTreeBuilder
import com.alext.rtree.core.geometry.Geometry

/**
 * Same as [RTree] with additional inverted [index] map for entry
 */
interface MultiIndexedRTree<T, G : Geometry> : RTree<T, G> {
  /**
   * Secondary index for entries.
   */
  val index: Map<T, List<Entry<T, G>>>
   ...
   */
  override fun add(entry: Entry<T, G>): MultiIndexedRTree<T, G>
   ...
   */
  override fun add(value: T, geometry: G): MultiIndexedRTree<T, G>
   ...
   */
  override fun add(entries: Iterable<Entry<T, G>>): MultiIndexedRTree<T, G>
   ...
   */
  override fun remove(entries: Iterable<Entry<T, G>>, all: Boolean): MultiIndexedRTree<T, G>
   ...
   * @param all if false deletes one if exists else deletes all
   */
  override fun remove(value: T, geometry: G, all: Boolean): MultiIndexedRTree<T, G>
   ...
   * @param all If false deletes one if exists else deletes all
   */
  override fun remove(entry: Entry<T, G>, all: Boolean): MultiIndexedRTree<T, G>

  companion object {

    fun <T, G : Geometry> builder(): Builder<T, G, MultiIndexedRTree<T, G>> {
      return ImmutableMultiIndexedRTreeBuilder()
    }
  }
}
